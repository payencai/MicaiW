		...
		if(obj instanceof JSONArray){
			...
			if(names.length > 1){
				...
				try {
					...
					Log.d("JSONUtil", ">>>>>>>JSONException:" + jsonEx.getMessage());
				}
			}
			...
			if (f.getType().equals(List.class)) {
				if (!obj.isNull(f.getName())) {
					...
					if (array instanceof JSONArray) {
						...
						// Edited by simon below ...
						CLJsonMapping jsonInfo = null;
						jsonInfo = f.getAnnotation(CLJsonMapping.class);
						if(jsonInfo == null) {
							...
							ArrayList<Object> children = new ArrayList<Object>(o.length());
							for(int i = 0;i < o.length();i ++){
								children.add(convertFromArray(Object.class,o,i));
							}
							f.set(t, children);
						}
					}
