...
public class NoHttp {
    ...
    private List<HttpCookie> mCookieList;   // Added by simon at 2018/1/24
     ...
     *
     */
    public static void setCookieList() {
        if(getInstance().mCookieList == null)
            getInstance().mCookieList = new ArrayList<>();
        else
            getInstance().mCookieList.clear();
        getInstance().mCookieList.addAll(getInstance().mCookieManager.getCookieStore().getCookies());
    }

    public static List<HttpCookie> getCookieList() {
        return getInstance().mCookieList;
    }
}
