...
//    }
    ...
    // Modify by simon at 2018/1/24
    public StringRequest(String url, RequestMethod requestMethod) {
        super(url, requestMethod);
        if((headers().getCookies() == null || headers().getCookies().size() == 0) && NoHttp.getCookieList() != null) {
            for (HttpCookie httpCookie: NoHttp.getCookieList()) {
                addHeader(httpCookie);
            }
        }
    }
